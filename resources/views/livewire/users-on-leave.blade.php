        ...
        <div class="col-span-1 font-light gap-y-4 md:gap-4 md:col-span-6 lg:col-span-6 xl:col-span-4">
            <div
                ...
                <div class="p-4 pr-6 leading-normal">
                    <div class="text-xl font-medium truncate dark:text-light">Title: {{$data->title}}
                    </div>
                    <div class="text-xl font-medium truncate dark:text-light">User: {{$data->requested_by}}
                    </div>
                    <div class="pb-2 text-xs font-semibold tracking-widest text-gray-500 uppercase truncate">
                        Commencing from:{{$data->from}}</div>
                        <div class="pb-2 text-xs font-semibold tracking-widest text-gray-500 uppercase truncate">
                        Ending:{{$data->to}}</div>
                    <div class="dark:text-gray-500">Reason:{{$data->message}}</div>
                </div>
            </div>
        </div>
        @empty
        <div class="col-span-10 p-6 mx-auto text-2xl text-white bg-primary-light">
            Great ! looks like we have a full house today.
        </div>
        @endforelse
    </div>
    {{-- <div class="flex items-center justify-center h-24 mt-6">{{ $leave->links('pagination-links') }}</div> --}}
</div>
